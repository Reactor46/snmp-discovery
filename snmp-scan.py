#!/usr/bin/env python
"""
Scan networks for snmp hosts and add them to Observium
Forked from LibreNMS
...
@maintainer Alexandre Nicolaie <alexandre.nicolaie@gmail.com>
"""
...
Result = namedtuple('Result', ['ip', 'hostname', 'outcome'])
...
def get_outcome_message(outcome):
    return {
        Outcome.UNDEFINED: 'Undefined',  # should not occur
        Outcome.ADDED: 'Device added',
        Outcome.UNPINGABLE: 'Device unpingable',
        Outcome.KNOWN: 'Device already known',
        Outcome.FAILED: 'Failed to add device',
        Outcome.TERMINATED: ''
    }[outcome]
...
def handle_result(data):
    if VERBOSE_LEVEL > 0:
        print('Scanned \033[1m{}\033[0m {}'.format(
            ("{} ({})".format(data.hostname, data.ip) if data.hostname else data.ip), get_outcome_message(data.outcome)))
    else:
...
def scan_host(ip):
    ...
    try:
        ...
        try:
            arguments = ['/usr/bin/env', 'php', 'add_device.php', hostname or ip]
            if args.ping:
                arguments.insert(3, args.ping)
            output = check_output(arguments).decode('utf-8')
            debug('Output for {}: {}'.format(hostname or ip, output), 2)
            if 'Already got device' in output:
                return Result(ip, hostname, Outcome.KNOWN)
            elif 'Could not ping' in output:
                return Result(ip, hostname, Outcome.UNPINGABLE)
            elif 'Devices success: 1' in output:
                return Result(ip, hostname, Outcome.ADDED)
            elif 'Devices failed: 1' in output:
                return Result(ip, hostname, Outcome.FAILED)
            else:
                return Result(ip, hostname, Outcome.UNDEFINED)
        except CalledProcessError as err:
            debug('Add device failed for {}'.format(hostname or ip), 2)
            return Result(ip, hostname, Outcome.FAILED)
...
if __name__ == '__main__':
    ...
    # Parse arguments #
    ###################
    parser = argparse.ArgumentParser(description='Scan network for snmp hosts and add them to Observium.', formatter_class=RawTextHelpFormatter)
    parser.add_argument('-N', '--network', action='append', nargs='*', type=str, required=True, help="""CIDR noted IP-Range to scan. Can be specified multiple times
...
Example: 192.168.0.1/32 will be treated as a single host address""")
    parser.add_argument('-E', '--exclude', action='append', nargs='*', type=str, help="""CIDR noted IP-Range to exclude from scan. Can be specified multiple times
Example: 127.0.0.0/8
Example: 192.168.0.0/31 will be treated as an RFC3021 p-t-p network with two addresses, 192.168.0.0 and 192.168.0.1
Example: 192.168.0.1/32 will be treated as a single host address""")
    parser.add_argument('-P', '--ping', action='store_const', const="-p", default="", help="""Add the device as an ICMP only device if it replies to ping but not SNMP.
Example: """ + __file__ + """ -P 192.168.0.0/24""")
    ...
    excargs = []
    for a in (args.exclude if args.exclude is not None else []):
        if type(a) is list:
            excargs += a
        else:
            excargs.append(a)
    ...
    if not netargs:
        parser.error('You must specify a network to scan')
    ...
    for net in excargs:
        try:
            ...
            parser.error('Invalid excluded network format {}'.format(e))
